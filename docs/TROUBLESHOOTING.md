...
### Disko завис при разметке диска

Самая частая причина — **остатки прошлой системы на диске**: GPT backup в
конце диска, активный swap, открытый LUKS, незакрытые LVM volume groups.

**Симптомы и причины:**

| Симптом | Причина | Что делать |
|---|---|---|
| `disko` вывел пару строк и завис без сообщения | `udevadm settle` ждёт устройство, или `wipefs` застрял на старой подписи | запустить `scripts/prep-disk.sh /dev/<disk>` |
| `Device or resource busy` | активный swap, LUKS или LVM с прошлой системы | `prep-disk.sh` закрывает всё это |
| `mkfs.btrfs: error initializing` | остатки старой BTRFS филсистемы | `wipefs -af` (есть в `prep-disk.sh`) |
| зависает на `mkfs.fat ... /dev/xxx2` | старая GPT таблица не обновилась в ядре | `sgdisk --zap-all` + `partprobe` (в `prep-disk.sh`) |
| `Permission denied` или похожие | ты запустил установку **не из root** | пересмотри — disko требует root |
| зависает на «свежем» диске прямо из коробки | редко — кривой кабель SATA, плохой контроллер USB-в-NVMe, или железо устройства | смени кабель/слот, проверь `dmesg \| tail -50` после зависания на kernel-ошибки |

**Алгоритм действий:**

1. Прервать зависший disko (Ctrl+C может не сработать — `pkill -9 disko` из другого TTY)
2. Получить диагностику:
   ```bash
   lsblk                      # видно ли диск, какие на нём остатки
   dmesg | tail -50           # последние ядерные сообщения, есть ли ошибки I/O
   journalctl -xe | tail -50  # системные ошибки
   ```
3. Проверить что `settings.disk` в `hosts/<host>/settings.nix` указывает на **правильный диск**:
   ```bash
   lsblk -d -o NAME,SIZE,MODEL,TRAN
   ```
   Особенно частая ошибка — указали `/dev/sda` а диск NVMe (`/dev/nvme0n1`).
4. Запустить подготовку:
   ```bash
   sudo bash scripts/prep-disk.sh /dev/<твой-диск>
   ```
5. Заново disko — на этот раз должно пройти.

Если после `prep-disk.sh` всё равно зависает — это уже железо или драйвер.
Проверь `dmesg | tail -100` на kernel-ошибки I/O, и попробуй с другим
кабелем / в другом слоте.
